  ...
  class SyncCaseComments < ::Jobs::Base
    def execute(args)
      ...
      posts =
        Post
          .joins(
            "LEFT JOIN post_custom_fields ON posts.id = post_custom_fields.post_id AND post_custom_fields.name = '#{::Salesforce::CaseComment::ID_FIELD}'",
          )
      ...
      posts.find_each { |post| ::Salesforce::CaseComment.new(salesforce_case.uid, post).create! }
    end
  end
end
