...
after_initialize do
  ...
  add_model_callback :user_profile, :after_save do
    if (lead_id = self.user.salesforce_lead_id) && saved_change_to_website? && self.website.present?
      Salesforce::Lead.update(lead_id, Website: self.website)
    end
  end
